...
export default function VehicleDetails() {
  ...
  // Theme-aware styles
  const editButtonStyle = themeId === 'modern'
    ? 'hover:bg-white/20 active:bg-white/30'
    : themeId === 'dark'
      ? 'hover:bg-gray-700 active:bg-gray-600'
      : 'hover:bg-gray-100 active:bg-gray-200';

  const statsBarBg = themeId === 'dark'
    ? 'bg-gray-800 border-gray-700'
    : themeId === 'modern'
      ? 'bg-white/80 backdrop-blur-sm border-white/50'
      : 'bg-white border-gray-100';

  const tabsBarBg = themeId === 'dark'
    ? 'bg-gray-800'
    : themeId === 'modern'
  ...
  return (
    <div className={`min-h-screen ${theme.pageBg} pb-8`}>
        ...
        rightAction={
          userIsAdmin && (
            <button
              ...
              className={`p-2 -mr-2 rounded-full transition-colors ${editButtonStyle}`}
            >
              <svg className={`w-5 h-5 ${theme.headerIcon}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className={`border-b px-4 py-3 ${statsBarBg}`}>
        <div className="flex items-center justify-between">
          ...
          {userIsAdmin ? (
            <div className="text-right">
              <p className={`text-xs ${theme.textSecondary} mb-1`}>Max. Gebot</p>
            ...
            <div className="text-right">
              <p className={`text-xs ${theme.textSecondary}`}>Deine Bewertung zählt</p>
            </div>
          )}
        </div>
        ...
        {userIsAdmin && (
          <div className={`flex items-center justify-between mt-2 pt-2 border-t ${themeId === 'dark' ? 'border-gray-700' : 'border-gray-100'}`}>
            <span className={`text-sm ${theme.textSecondary}`}>Profit bei aktuellem Gebot</span>
      ...
      <div className={`sticky top-14 z-30 px-4 py-3 ${tabsBarBg}`}>
...
function InfoRow({ label, children, mono, highlight }) {
  return (
    <div className="flex justify-between items-center py-2 border-b border-gray-50 last:border-0 last:pb-0 first:pt-0">
      ...
      <span className={`font-medium tabular-nums ${mono ? 'font-mono text-sm' : ''} ${highlight ? 'text-blue-600' : 'text-gray-900'}`}>
        {children}
      </span>
    </div>
  );
...
function AdminReviewsTab({ vehicle, onUpdate }) {
  ...
  // SVG Icons (mit aria-hidden für dekorative Icons)
  const UsersIcon = () => (
          ...
          {vehicle.reviews.map((review) => (
            <Card key={review.id}>
              ...
              <button
                ...
                aria-label={`Bewertung von ${review.mechanicName} löschen`}
              >
                <TrashIcon />
                <span>Löschen</span>
              </button>
...
function CostRow({ label, value, sublabel, bold }) {
  return (
    <div className="flex justify-between items-center">
      ...
      <span className={`tabular-nums ${bold ? 'font-semibold text-gray-900' : 'text-gray-900'}`}>
